...
% extract free parameters from input matrix p:
...
% fixed parameters before optimization:
k_decline = 0.345;
c_plt = 1.4e10;
CD34 = 3.5e6;
...
% reformat the parameters to be used to solve ODE:
parameters = [p_c;a_c;k_p,k_a,d_plt,k_decline,k_shed, 0];

% Calculate healthy equilibrium: (Equ.11 to 17):
...
% Equ. 18, objective function to be minimized:
% Object function part one, difference between healthy equilibrium state with reference cell count (Equ. 18):
obj_equ = ((c_equ(6)-1.6e7)/1e7)^2+((c_equ(5)-1.6e7)/1e7)^2+((c_equ(4)-2e7)/1e7+(c_equ(3)-2e7)/1e7)^2+((c_equ(2)-4e6)/1e6)^2+((c_equ(1)-1.4e5)/1e5)^2;

% solve ODE function with input parameters:
tspan = [0 200]; % days
c0 = [CD34*0.0408, CD34*0.072, CD34*0.284, CD34*0.148, 0, 0, 0, plt_start];
[t, c] = ode45(@(t,c) ODE( t, c, parameters), tspan, c0);

% initialize matrix to store objective function value:
obj_sim = 0;

% Objective function part two, difference between platelets clinical data with simulation results (Equ.18):
for i = 1:length(Days)
